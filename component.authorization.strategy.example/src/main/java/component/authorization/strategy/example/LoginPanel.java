...
public class LoginPanel extends Panel {
	...
	public LoginPanel(String id) {
		...
		final CompoundPropertyModel<LoginModel> cpm = new CompoundPropertyModel<LoginModel>(
				model);

		final Form<LoginModel> form = new Form<LoginModel>("form", cpm);
		add(form);

		final TextField<String> username = new RequiredTextField<String>(
				"username");
		form.add(username);
		...
		final Button loginButton = new Button("loginButton") {
			...
			public void onSubmit() {
				if (((WicketSession) getSession()).login(model.getUsername(),
						model.getPassword())) {
